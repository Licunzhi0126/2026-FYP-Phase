"""Unified simulation visualization pipeline and standalone CLI."""
...
from .figure3_simulation import (
    build_figure3_data,
    plot_figure3a,
    plot_figure3b,
    plot_figure3c,
...
from .simulation_imbalance import (
...
def run_simulation_visualization(
    ...
    *,
    ...
    """Generate every requested simulation figure from one aligned data bundle."""
    ...
    if make_summary:
        ...
        _attempt(
            "summary/grn_metrics",
            lambda: plot_grn_metrics(bundle, summary_dir / "grn_metrics.png", dpi),
            outputs,
            failures,
        )
        _attempt(
        ...
        for chromosome in bundle.chromosomes:
            ...
            if make_chromosome:
                chromosome_tasks = (
                    ...
                    ("phase_contrast_track", plot_phase_contrast_track),
                    ("gene_correlation_track", plot_gene_correlation_track),
                    ("phase_correlation_heatmap", plot_phase_correlation_heatmap),
                    ("delta_correlation", plot_delta_correlation),
                )
                for suffix, function in chromosome_tasks:
                    _attempt(
                        f"{chromosome}/{suffix}",
