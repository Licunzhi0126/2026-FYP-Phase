...
from .diagnostics import EPS, VisualizationBundle
...
def _bundle_imbalances(bundle: VisualizationBundle, genes: list[str]):
    ...
    predicted = np.where(
        bundle.observed_mask[genes].to_numpy(dtype=bool),
        predicted,
        np.nan,
    )
...
def write_gene_level_imbalance(bundle: VisualizationBundle, path: Path) -> Path:
    ...
    frame = pd.DataFrame(
        {
            ...
            "truth_mean_signed_imbalance": np.nanmean(truth, axis=0),
            "truth_mean_absolute_imbalance": np.nanmean(np.abs(truth), axis=0),
            "pred_mean_signed_imbalance": np.nanmean(predicted, axis=0),
            "pred_mean_absolute_imbalance": np.nanmean(np.abs(predicted), axis=0),
            "mean_residual": np.nanmean(predicted - truth, axis=0),
        }
    )
...
def plot_chromosome_imbalance_track(
    ...
    pred_mean = np.nanmean(predicted, axis=0)
...
def _pseudobulk_imbalance(bundle: VisualizationBundle, genes: list[str]):
    ...
    for context in contexts:
        ...
        observed = bundle.observed_mask.loc[mask, genes]
        pred_a = bundle.pred_maternal.loc[mask, genes].where(observed)
        pred_b = bundle.pred_paternal.loc[mask, genes].where(observed)
        truth_rows.append(
            signed_imbalance(
...
def _plot_imbalance_heatmap(
    bundle: VisualizationBundle, genes: list[str], title: str, path: Path, dpi: int
) -> Path:
...
def plot_genome_imbalance_heatmap(
    bundle: VisualizationBundle, path: Path, dpi: int
) -> Path:
    genes = bundle.ordered_genes()
    contexts, truth, predicted = _pseudobulk_imbalance(bundle, genes)
    residual = predicted - truth
