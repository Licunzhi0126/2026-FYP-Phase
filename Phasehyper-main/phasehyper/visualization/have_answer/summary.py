"""Summary figures for expression and GRN evaluation."""
...
from .diagnostics import VisualizationBundle, safe_corr
...
def _grouped_bars(ax, frame: pd.DataFrame, metrics, labels, title) -> None:
    ...
    ax.set_title(
        title, color=INK, fontsize=15, loc="left", pad=16, fontweight="bold"
    )
    style_axis(ax)


def plot_expression_metrics(bundle: VisualizationBundle, path: Path, dpi: int) -> Path:
...
def plot_grn_metrics(bundle: VisualizationBundle, path: Path, dpi: int) -> Path:
