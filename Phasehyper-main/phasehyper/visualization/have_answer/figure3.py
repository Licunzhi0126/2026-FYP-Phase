"""Saber-style Figure 3 panels for results with known ground truth."""
...
from .diagnostics import VisualizationBundle
from .imbalance import signed_imbalance
...
def _orient_pair(pair, truth_a, truth_b, observed_mask):
...
def build_figure3_data(bundle: VisualizationBundle) -> Figure3Data:
    observed_full = bundle.observed_mask.to_numpy(dtype=bool)
    minimum_support = min(10, len(bundle.cells))
    eligible = observed_full.sum(axis=0) >= minimum_support
    if not eligible.any():
        raise ValueError(
            f"no genes have at least {minimum_support} observed truth values"
        )
    gene_names = np.asarray(bundle.genes, dtype=object)[eligible].tolist()
    combined = bundle.combined.to_numpy()[:, eligible]
    truth_m = bundle.maternal.to_numpy()[:, eligible]
    truth_p = bundle.paternal.to_numpy()[:, eligible]
    observed = observed_full[:, eligible]
    hyper_a = bundle.pred_maternal.to_numpy()[:, eligible]
    hyper_b = bundle.pred_paternal.to_numpy()[:, eligible]
    methods = {
        "HyperPhase": (
            hyper_a,
            hyper_b,
        ...
        "RandomSplit": _orient_pair(
            saber.baseline_random_split(combined, bundle.seed),
            truth_m,
            truth_p,
            observed,
        ),
        "NMF2Factor": _orient_pair(
            saber.baseline_nmf2(combined, bundle.seed),
            truth_m,
            truth_p,
            observed,
        ),
    ...
    pred_imb_observed = np.where(observed, pred_imb, np.nan)
    truth_magnitude = np.nanmean(np.abs(truth_imb), axis=0)
    pred_magnitude = np.nanmedian(np.abs(pred_imb_observed), axis=0)
    low_cut, high_cut = np.nanquantile(truth_magnitude, [1 / 3, 2 / 3])
    groups = np.where(
    ...
    hyper_gene_mse = 0.5 * (
        np.nanmean((methods["HyperPhase"][0] - truth_m) ** 2, axis=0)
        + np.nanmean((methods["HyperPhase"][1] - truth_p) ** 2, axis=0)
    )
    baseline_gene_mse = 0.5 * (
        np.nanmean((methods["combined/2"][0] - truth_m) ** 2, axis=0)
        + np.nanmean((methods["combined/2"][1] - truth_p) ** 2, axis=0)
    )
    gene_level = pd.DataFrame(
        {
            "gene_id": gene_names,
            ...
            mse_rows.append(
                {
                    ...
                    "phasefit_mse": 0.5
                    * (
                        np.nanmean((first[mask] - truth_m[mask]) ** 2)
                        + np.nanmean((second[mask] - truth_p[mask]) ** 2)
                    ),
                }
            )
    ...
    high_threshold = np.nanquantile(
        truth_magnitude, saber.HIGH_IMB_QUANTILE
    )
    detection_scores = pd.DataFrame(
        {
            "gene_id": gene_names,
            ...
            "truth_mean_absolute_imbalance": truth_magnitude,
            "HyperPhase imbalance": np.nanmean(
                np.abs(pred_imb_observed), axis=0
            ),
            "phase separability": np.nanmean(
                np.where(
                    observed,
                    np.abs(
                        methods["HyperPhase"][0] - methods["HyperPhase"][1]
                    ),
                    np.nan,
                ),
                axis=0,
    ...
    orientation = pd.DataFrame(
        saber.orientation_audit(
            bundle.pred_a_raw.to_numpy()[:, eligible],
            bundle.pred_b_raw.to_numpy()[:, eligible],
            truth_m,
            truth_p,
            "phasehyper",
            observed,
