"""Simulation data alignment and chromosome-level diagnostic figures."""
...
def load_simulation_bundle(sim_dir: Path, result_dir: Path) -> SimulationBundle:
    ...
    maternal = _read_matrix(sim_dir / "groundtruth" / "maternal_true_expression.csv")
    paternal = _read_matrix(sim_dir / "groundtruth" / "paternal_true_expression.csv")
    ...
    if not cells or not genes:
        raise ValueError("simulation inputs have no common cells or genes")
...
def write_aligned_annotation(bundle: SimulationBundle, path: Path) -> Path:
...
def plot_phase_expression_heatmap(
    ...
    panels = [
        (truth_m, "Truth maternal", "viridis"),
        (pred_m, "Pred maternal", "viridis"),
        (pred_m - truth_m, "Residual (pred − truth)", "RdBu_r"),
        (truth_p, "Truth paternal", "viridis"),
        (pred_p, "Pred paternal", "viridis"),
...
def plot_phase_contrast_track(
    ...
    pred_m = bundle.pred_maternal[genes].mean(axis=0).to_numpy()
    pred_p = bundle.pred_paternal[genes].mean(axis=0).to_numpy()
    ...
    for ax, values, label, color in (
        (axes[0], truth_ratio, "Truth M/P", INK_2),
        (axes[1], pred_ratio, "Predicted M/P", POS),
    ):
...
def plot_phase_correlation_heatmap(
    ...
    pred_m = correlation_matrix(bundle.pred_maternal[genes].to_numpy())
    truth_p = correlation_matrix(bundle.paternal[genes].to_numpy())
    pred_p = correlation_matrix(bundle.pred_paternal[genes].to_numpy())
    panels = [
        (truth_m, "Truth maternal corr"),
        (pred_m, "Pred maternal corr"),
        (pred_m - truth_m, "Maternal residual"),
        (truth_p, "Truth paternal corr"),
        (pred_p, "Pred paternal corr"),
        (pred_p - truth_p, "Paternal residual"),
    ]
...
def _delta_correlation_matrices(
    bundle: SimulationBundle, genes: Sequence[str]
    ...
    predicted = correlation_matrix(
        bundle.pred_maternal[list(genes)].to_numpy()
    ) - correlation_matrix(bundle.pred_paternal[list(genes)].to_numpy())
...
def plot_gene_detail(bundle: SimulationBundle, gene: str, path: Path, dpi: int) -> Path:
    ...
    for values, label, color, ls in (
        (truth_m, "truth maternal", INK, "-"),
        (truth_p, "truth paternal", INK_2, "--"),
        (pred_m, "pred maternal", POS, "-"),
        (pred_p, "pred paternal", NEG, "--"),
    ):
